...
Ключевое отличие — транспорт и формат обмена stdio (MCP нативно):
- Общение по stdin/stdout.
- Формат: JSON‑RPC 2.0, построчно или через Content-Length.
- Нет URL/методов HTTP, нет заголовков, нет статусов HTTP

Используют не только из IDE, а как локальный сервис‑утилиту:
...
```json
{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"search_docs","arguments":{"query":"Performance Enhancement"}}}
```

## HTTP‑режим

http — нужен отдельный прокси/адаптер, который преобразует HTTP‑запросы в MCP‑вызовы. Удобно для внешних клиентов и инструментов (например, HTTPYac), но требует поднять дополнительный процесс и порт.
